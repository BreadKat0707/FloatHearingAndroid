...
interface SongDao {
    ...
    fun getAllSongsBySource(source: Int): Flow<List<Song>>
    ...
    @Query(
        "SELECT * FROM songs WHERE source = :source AND (title LIKE '%' || :query || '%' OR artist LIKE '%' || :query || '%' OR album LIKE '%' || :query || '%') ORDER BY title COLLATE NOCASE"
    )
    ...
    fun getSongsByAlbumAndSource(album: String, albumArtist: String?, source: Int): Flow<List<Song>>
    ...
    suspend fun getAllSongsSnapshotBySource(source: Int): List<Song>

    @Query("DELETE FROM songs WHERE path NOT IN (:paths)")
    suspend fun deleteAllNotInPaths(paths: Set<String>)

    @Query("DELETE FROM songs WHERE source = :source AND path NOT IN (:paths)")
    suspend fun deleteSourceNotInPaths(paths: Set<String>, source: Int)
    ...
    /**
     * Replaces only the songs owned by [source], keeping songs from the other source.
     */
    @androidx.room.Transaction
    suspend fun replaceSource(songs: List<Song>, source: Int): Int {
        val newPaths = songs.map { it.path }.toSet()
        val existing = getAllSongsSnapshotBySource(source).associateBy { it.path }
        val removed = existing.keys.count { it !in newPaths }
        ...
        if (newPaths.isEmpty()) {
            ...
            deleteSourceNotInPaths(newPaths, source)
        }
        insertAll(merged)
        return removed
    }
